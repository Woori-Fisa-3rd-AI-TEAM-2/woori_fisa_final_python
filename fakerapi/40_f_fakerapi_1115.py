...
merchant = {
    ...
    '03': {
        ...
        'stores': ['배달의민족', '본죽', '이삭토스트', '김밥천국', '롤링파스타', '맥도날드', '투다리', '역전할머니맥주', 'VIPS', '채선당', '원할머니보쌈', '삼원가든', '명동칼국수'],
    ...
    '11': {
        ...
        'stores':['하이패스', '한국도로공사', '서울고속도로', '서울외곽순환도로', '인천국제공항고속도로', '경부고속도로', '영동고속도로', '서해안고속도로', '호남고속도로', '지마켓_주유권', '11번가_주유권'],
...
def generate_expense_data(num_records=20000):
    ...
    # 연간 목표 총액 설정
    annual_target = 36_000_000  # - 40대  : 36,000,000  ( 중위값) 월: 3,000,000

    # 정기 결제 항목 정의
    subscriptions = {
        'Cafe': {'amount': 32000, 'interval': 30, 'category': '02', 'category_name': '카페/디저트'},  # 4000원 * 4회 * 4주
        # 'Netflix': {'amount': 17000, 'interval': 180, 'category': '05', 'category_name': '영화/문화'},
        'Youtube': {'amount': 7900, 'interval': 30, 'category': '05', 'category_name': '영화/문화'},
        'KT': {'amount': 75000, 'interval': 30, 'category': '10', 'category_name': '통신'}, # 통신 75000
        'Transportation': {'amount': 62000, 'interval': 30, 'category': '13', 'category_name': '교통(대중)'}  # 월 62,000
...
for merchant, stats in sub_stats.iterrows():
    ...
    print(f"{merchant:15} - {count:3d}회 총 {sum_amount:,.2f}원")